...
(function () {
  ...
  // Preload the public, non-secret configuration so a failed Google login can
  // explain whether account storage or Google configuration is missing.
  getConfig();
  ...
  window.fetch = async function (input, init) {
    ...
    const body = JSON.stringify({ error, code: config?.accounts_enabled === false ? 'ACCOUNT_STORAGE_NOT_CONFIGURED' : 'GOOGLE_CONFIGURATION_OR_TOKEN_ERROR' });
