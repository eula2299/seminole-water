...
function buildResidentReport(data){
 ...
 return {version:'resident-report/1',headline,summary,address:data.address?.matched_address||null,address_choices:data.address?.candidates||[],providers,provider_ambiguity:candidates.length>1||data.provider?.conflict===true,private_well:privateWell,findings,detected_substances:detectedNames.size,compliance,actions,service_line:service||null,location:data.address?.geography||{},scope_note:'Historical water-system and nearby environmental records help you decide what to check. They cannot certify the water at a particular tap today.',current_advisories_checked:false,household_safety:'not-determined',generated_at:data.generated_at};
}
