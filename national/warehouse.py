...
def canonical_sql(columns,family):
 ...
 CASE WHEN {qualifier}='=' AND isfinite({value}) AND {value}>=0 THEN {value} ELSE NULL END AS detected_value,
...
def process_member(source_file,parquet_path,summary_db,family,member):
 ...
 try:
  ...
  return {'member':member,'raw_rows':raw_count,'distinct_source_rows':count,'invalid_identity_date_rows':invalid,'sha256':digest(parquet_path),'bytes':parquet_path.stat().st_size}
 finally:con.close()
...
def status():
 ...
 s.update(retained_source_records=count,raw_rows=sum(x['raw_rows'] for x in receipts),published_archives=len(receipts),stored_bytes=sum(x['stored_bytes'] for x in receipts),target_met=count>=s['target_minimum_records'],count_definition='distinct source-field rows within each published archive member; not independently sampled households; cross-source independence not certified',household_safety_certified=False,coverage_complete=False)
 return s
...
def ingest(store,item):
 ...
 with tempfile.TemporaryDirectory(prefix='ingest-',dir=ROOT) as td:
  ...
  if existing and existing['sha256']==receipt['sha256']:
   log('unchanged',source=item['id']);return
  base='v1/data/'+receipt['sha256'];summary=work/'summary.sqlite';parts=[];skipped=[];expanded=0
  ...
  receipt.update(parts=parts,skipped_members=skipped,raw_rows=sum(x['raw_rows'] for x in parts),distinct_source_rows=sum(x['distinct_source_rows'] for x in parts),summary_key=base+'/summary.sqlite',summary_sha256=digest(summary))
  receipt['stored_bytes']=receipt['archive_bytes']+summary.stat().st_size+sum(x['bytes'] for x in parts)
  if status()['stored_bytes']+receipt['stored_bytes']>MAX_TOTAL:raise ValueError('Persistent storage budget exceeded; no publication performed')
...
def pws_report(pwsid):
 ...
 return {'pwsid':pwsid,'status':'records-returned' if summaries else 'no-records-in-loaded-archives','summaries':summaries,'sources':sources,'archives_checked':len(receipts),'scope':'public-system-samples-not-household','current_safety':'not-determined','cross_source_sample_independence_certified':False,'historical_data':True,'coverage_complete':False}
