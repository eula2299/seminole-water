...
test('private-well snapshot comes first and detailed next steps remain available',async()=>{const headings=await render(true);assert.match(headings[1],/YOUR WATER SNAPSHOT/);const actions=headings.indexOf('What to do next'),well=headings.indexOf('What to test in your well');assert.ok(actions>1&&well>actions);assert.ok(!headings.includes('Water notices to check now'));});
