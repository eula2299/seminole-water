  ...
  async createPasswordUser({ name, email, password }) {
    ...
    try {
      const result = await this.query(
        `INSERT INTO community_users(email,name,password_hash) VALUES($1,$2,$3) RETURNING *`,
    ...
    if (existing.rows[0]) {
      ...
      const updated = await this.query(
        `UPDATE community_users SET google_sub=COALESCE(google_sub,$2), name=CASE WHEN name='' THEN $3 ELSE name END,
         picture_url=COALESCE(NULLIF($4,''),picture_url), last_login_at=NOW() WHERE id=$1 RETURNING *`,
        [row.id, sub, String(name || '').trim().slice(0, 100), String(picture || '').slice(0, 500)]
      );
      return updated.rows[0];
    }
    const created = await this.query(
      `INSERT INTO community_users(email,name,google_sub,picture_url) VALUES($1,$2,$3,$4) RETURNING *`,
