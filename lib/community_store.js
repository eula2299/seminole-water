...
class CommunityStore {
  ...
  async addAccessEvent({ state = '', county = '', higherBarrierContext = false, freeOptions = 0, recordsTranslated = 0, gapsIdentified = 0 }) {
    const bounded = value => Math.max(0, Math.min(10000, Number.isFinite(Number(value)) ? Math.floor(Number(value)) : 0));
    await this.query(
      `INSERT INTO water_access_events(state,county,higher_barrier_context,free_options,records_translated,gaps_identified) VALUES($1,$2,$3,$4,$5,$6)`,
      [String(state || '').toUpperCase().slice(0,2), String(county || '').slice(0,120), !!higherBarrierContext, bounded(freeOptions), bounded(recordsTranslated), bounded(gapsIdentified)]
    );
  }
