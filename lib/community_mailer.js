...
class CommunityMailer {
  ...
  get configured() { return !!this.transporter || !!process.env.RESEND_API_KEY; }

  async send({ to, subject, text, html, replyTo }) {
    if (process.env.RESEND_API_KEY) {
      const from = process.env.MAIL_FROM || 'IsMyWaterOK <onboarding@resend.dev>';
      ...
      return { sent: true, messageId: data.id || null, provider: 'resend' };
    }
